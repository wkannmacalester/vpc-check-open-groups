...
################################
##  Rule Config by Will Kann  ##
################################

#Rule Name: vpc-check-open-groups.py

#Description: Checks for any open security groups attached to EC2 instances

#Trigger: Configuration changes to EC2 security groups

#Reports on: AWS::EC2::Instance
...
def findOpenGroup(ip_permissions):
    """Check a list of given ipv4 permissions to see if any ports allow connections to 0.0.0.0./0
    ip_permissions - the list of ip permissions taken from a security group
    """
    groupOpen = False
    for permission in ip_permissions:
        if next((r for r in permission["IpRanges"]
                  if '0.0.0.0/0' in r["CidrIp"]), None): groupOpen = True
    return groupOpen
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    securityGroups = configuration_item['configuration'].get("securityGroups")

    ec2 = boto3.resource("ec2")
    for securityGroup in securityGroups:
        #get permission list from the security group
        ip_permissions = ec2.SecurityGroup(securityGroup["groupId"]).ip_permissions
        groupOpen = findOpenGroup(ip_permissions)
        if (groupOpen): #if there's an open port then the group is not compliant
            return build_evaluation_from_config_item(configuration_item, 'NON_COMPLIANT', annotation='Security group allows connections from 0.0.0.0/0')

    return build_evaluation_from_config_item(configuration_item, 'COMPLIANT', annotation='Security group does not allow connections from 0.0.0.0/0')
